...
void print_block_data(struct super_block *sb, sector_t blocknr,
		      unsigned char *data, int start, int len)
{
	print_hex_dump(KERN_ERR, "EXT4-fs data: ", DUMP_PREFIX_OFFSET,
		       16, 1, data + start, len, false);
}
...
{
	if (!iloc.bh) {
		ext4_msg(sb, KERN_ERR, "inode location has no buffer");
		return;
	}
	ext4_msg(sb, KERN_ERR, "inode location block=%llu offset=%lu",
		 (unsigned long long)iloc.bh->b_blocknr,
		 (unsigned long)iloc.offset);
}
#include "mballoc.h"
#include "fsmap.h"
