...
/* Compatibility for Samsung's pre-upstream fscrypt page helpers. */
#define FSCRYPT_DUN(inode, index) \
	((((u64)(inode)->i_ino & 0xffffffffULL) << 32) | \
	 ((u64)(index) & 0xffffffffULL))
#define FSCRYPT_PG_DUN(inode, page) FSCRYPT_DUN(inode, (page)->index)
...
static inline void fscrypt_pullback_bio_page(struct page **pagep,
					     bool restore)
{
	struct page *bounce = *pagep;

	if (!fscrypt_is_bounce_page(bounce))
		return;
	*pagep = fscrypt_pagecache_page(bounce);
	if (restore)
		fscrypt_free_bounce_page(bounce);
}
...
static inline struct page *fscrypt_encrypt_page(const struct inode *inode,
						 ...
						 gfp_t gfp_flags)
{
	return fscrypt_encrypt_pagecache_blocks(page, len, offs, gfp_flags);
}
...
/*
 * Samsung SDP / inline encryption compatibility stubs.
 * These are needed by security/sdp/dd_core.c which was written against an
 * older fscrypt stack that exposed inline-crypt helpers. Inline encryption
 * is not enabled in this tree, so they are provided as no-ops.
 */
#define fscrypt_set_bio_cryptd(inode, bio) fscrypt_set_bio_cryptd_dun(inode, bio, 0)
static inline void fscrypt_set_bio_cryptd_dun(const struct inode *inode,
