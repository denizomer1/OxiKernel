					...
					const char *buf, size_t count)
{
	...
	case SIOP_EVENT:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			if (battery->pdata->siop_event_check_type & SIOP_EVENT_WPC_CALL) {
				if (x & SIOP_EVENT_WPC_CALL) {
					...
					pr_info("%s : undefined SIOP EVENT 0x%x\n", __func__, battery->siop_event);
				}
			}
			ret = count;
		}
	...
	case WC_TX_EN:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			...
			if (is_wireless_type(battery->cable_type)) {
				...
				return count;
#if 0
			} else if (is_wired_type(battery->cable_type) && !is_hv_wire_type(battery->cable_type)) {
				pr_info("@Tx_Mode %s : Can't enable Tx mode during 5V charging\n", __func__);
				return count;
#endif
			} else {
	...
	case EXT_EVENT:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			dev_info(battery->dev,
				"@Tx_mode %s: ext event 0x%x \n", __func__, x);
			sec_bat_get_ext_event(battery, x);
			ret = count;
		}
		break;
	case FACTORY_MODE_RELIEVE:
