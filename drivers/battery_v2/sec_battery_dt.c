		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,wc_tx_limit_curr",
			&pdata->wc_tx_limit_curr);
	if (ret) {
		pdata->wc_tx_limit_curr = 700;
		pr_err("%s: wc tx limit current is Empty. set %dmA\n", __func__, pdata->wc_tx_limit_curr);
	}
